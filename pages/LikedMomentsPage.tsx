...
interface LikedMomentsPageProps {
    onBack: () => void;
}

const LikedMomentsPage: React.FC<LikedMomentsPageProps> = ({ onBack }) => {
    const { token } = useAuth();
    ...
    useEffect(() => {
        if (!token) return;
        const loadLikedMoments = async () => {
            try {
                const data = await momentsApi.getLikedMoments(token);
                setMoments(data);
            } catch (error) {
                console.error('Failed to load liked moments:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="w-full h-full bg-[#121212] flex flex-col">
            ...
            <div className="h-[60px] flex items-center justify-between px-4 border-b border-white/10 bg-[#1E1E1E]">
                ...
                <h1 className="text-white text-lg font-medium">My Likes</h1>
            ...
            <div className="flex-1 overflow-y-auto p-4 pb-24">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                        ...
                        <p>No liked moments yet</p>
                        <p className="text-sm mt-2">Start liking posts to see them here</p>
                    </div>
                ) : (
                                    ...
                                    <div className="p-3">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-1 text-[#FF00FF]">
                                                <Heart className="w-3 h-3 fill-[#FF00FF]" />
...
export default LikedMomentsPage;
